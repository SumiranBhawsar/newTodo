...
import { RootState } from "@/lib/store"; // Update this to your actual store path
...
const TodoList = () => {
  ...
  return (
    <div
      className="mt-4 w-full max-h-96 overflow-y-auto space-y-3"
    ...
    >
      ...
      todos?.map((todo: Todo) => (
        <div
        key={todo.id}
        className="bg-gray-800 p-4 rounded-lg shadow space-y-1 relative"
        >
        {/* Edit & Delete Buttons */}
        ...
        <p className="text-gray-400">{todo.content}</p>
        {todo.dueDate && (
